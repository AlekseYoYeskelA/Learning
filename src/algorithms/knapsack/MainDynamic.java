...
public class MainDynamic {
    ...
    private static void traceResult(int[][] A, int[] weigth, int k, int s, ArrayList<Integer> result) {
        if (A[k][s] == 0) {
            return;
        }
        if (A[k - 1][s] == A[k][s]) {
            traceResult(A, weigth, k - 1, s, result);
        } else {
            traceResult(A, weigth, k - 1, s - weigth[k - 1], result);
            result.addFirst(k);
        }
    }
}
